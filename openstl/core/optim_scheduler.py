...
def get_optim_scheduler(args, epoch, model, steps_per_epoch):  # 根据args获取optimizer、lr_scheduler
    opt_lower = args.opt.lower()  # e.g.: sgd, adam, lookahead_sgd, lookahead_adam
    ...
    if args.filter_bias_and_bn:
        if hasattr(model, 'no_weight_decay'):
            ...
            skip = {}
        parameters = get_parameter_groups(model, weight_decay, skip)  # 没有weight decay的参数: 只有一个axis的、参数名以`.bias`结尾的、skip中指定的
        weight_decay = 0.
    else:
