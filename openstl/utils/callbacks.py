...
class EpochEndCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        log = f"Epoch {trainer.current_epoch}: Lr: {lr:.7f}"
        if hasattr(self, 'avg_train_loss'):
            log += f" | Train Loss: {self.avg_train_loss:.7f}"
        log += f" | Vali Loss: {avg_val_loss:.7f}"
        # if 'val_mse' in trainer.callback_metrics:
        #     log += f" | Vali MSE: {trainer.callback_metrics['val_mse']:.2f}"
        # if 'val_ssim' in trainer.callback_metrics:
        #     log += f" | Vali SSIM: {trainer.callback_metrics['val_ssim']:.4f}"
        # if 'val_psnr' in trainer.callback_metrics:
        #     log += f" | Vali PSNR: {trainer.callback_metrics['val_psnr']:.2f}"
