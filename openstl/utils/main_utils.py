...
def update_config(args, config, exclude_keys=list()):
    # 合并args和config
    # exclude_keys指明的key -> config优先级更高
    # exclude_keys未指明的key -> args优先级更高
    # 综上所述, 该方法的作用是用args覆盖config, 当出现了覆盖的情况时, 把覆盖的key和value打印出来(不包括args有, 但是config没有的情况);
    # 也就是说下面的original code的注释把该方法的覆盖和被覆盖的对象写反了
